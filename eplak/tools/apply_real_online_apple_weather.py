...
print("--- [1] Updating modules/weather-3d.js with 100% Live Online Engine & Feathered Organic Clouds ---")
...
(function () {
  ...
  function renderStageHtml(container) {
    ...
    var html = ''
      ...
      +         '</div>'
      +         '<div class="apple-time-pill" id="appleTimePill">' + (isEn ? 'Online Station' : 'ایستگاه آنلاین') + '</div>'
      +       '</div>'
  ...
  /* ────────── ساخت ابرهای فوق‌العاده زیبا، ارگانیک و محو وکتوری ────────── */
  function getOrganicCloudsHtml(isDay, isRain) {
    // ابرهای ارگانیک وکتوری با فیلتر بلور ملایم، گرادینت‌های چندلایه و لبه‌های نرم و طبیعی
    var cloudGrad1 = isDay
      ? '<radialGradient id="acG1" cx="45%" cy="30%" r="65%"><stop offset="0%" stop-color="rgba(255,255,255,0.92)"/><stop offset="55%" stop-color="rgba(240,248,255,0.78)"/><stop offset="85%" stop-color="rgba(215,230,250,0.35)"/><stop offset="100%" stop-color="rgba(200,220,245,0)"/></radialGradient>'
      : '<radialGradient id="acG1" cx="45%" cy="30%" r="65%"><stop offset="0%" stop-color="rgba(165,190,225,0.65)"/><stop offset="55%" stop-color="rgba(110,140,180,0.45)"/><stop offset="85%" stop-color="rgba(70,95,135,0.2)"/><stop offset="100%" stop-color="rgba(50,75,115,0)"/></radialGradient>';

    if (isRain) {
      cloudGrad1 = isDay
        ? '<radialGradient id="acG1" cx="45%" cy="30%" r="65%"><stop offset="0%" stop-color="rgba(130,145,170,0.85)"/><stop offset="60%" stop-color="rgba(95,110,135,0.65)"/><stop offset="100%" stop-color="rgba(70,85,110,0)"/></radialGradient>'
        : '<radialGradient id="acG1" cx="45%" cy="30%" r="65%"><stop offset="0%" stop-color="rgba(45,60,85,0.88)"/><stop offset="60%" stop-color="rgba(28,40,62,0.68)"/><stop offset="100%" stop-color="rgba(15,25,45,0)"/></radialGradient>';
    }
    ...
    var svg1 = ''
      ...
      +   '<defs>'
      +     cloudGrad1
      +     '<filter id="cloudSoftBlur1" x="-20%" y="-20%" width="140%" height="140%"><feGaussianBlur stdDeviation="3.5"/></filter>'
      +   '</defs>'
    ...
    var svg2 = ''
      + '<svg class="apple-cloud-organic ac-org-2" viewBox="0 0 190 70" fill="none" xmlns="http://www.w3.org/2000/svg">'
      +   '<defs>'
      +     cloudGrad1
      +     '<filter id="cloudSoftBlur2" x="-20%" y="-20%" width="140%" height="140%"><feGaussianBlur stdDeviation="3"/></filter>'
      +   '</defs>'
      +   '<g filter="url(#cloudSoftBlur2)">'
      +     '<ellipse cx="50" cy="44" rx="38" ry="18" fill="url(#acG1)"/>'
      +     '<ellipse cx="96" cy="36" rx="46" ry="24" fill="url(#acG1)"/>'
      +     '<ellipse cx="138" cy="42" rx="36" ry="17" fill="url(#acG1)"/>'
      +     '<ellipse cx="76" cy="26" rx="28" ry="18" fill="url(#acG1)"/>'
      +     '<ellipse cx="114" cy="28" rx="30" ry="18" fill="url(#acG1)"/>'
      +   '</g>'
      + '</svg>';
  ...
  function applyWeatherState() {
    ...
    if (timePill) {
      var now = new Date();
      var timeStr = now.toLocaleTimeString(isEn ? 'en-US' : 'fa-IR', { hour: '2-digit', minute: '2-digit' });
      timePill.textContent = (isEn ? 'Live ' + timeStr : 'زنده • ' + timeStr);
    }
...
with open('/home/user/eplak/eplak-fixed/modules/weather-3d.js', 'w', encoding='utf-8') as f:
    f.write(new_weather_js)

print("   -> modules/weather-3d.js updated successfully!")
