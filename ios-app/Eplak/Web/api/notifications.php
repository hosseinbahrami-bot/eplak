...
try {
    ...
    if ($action === 'read' && $phone !== '') {
        ...
        if ($id > 0) {
            $stmt = $pdo->prepare('UPDATE notifications SET read_flag = 1 WHERE id = :id AND user_phone = :phone');
            $stmt->execute([':id' => $id, ':phone' => $phone]);
        } else {
            $stmt = $pdo->prepare('UPDATE notifications SET read_flag = 1 WHERE user_phone = :phone');
    ...
    if ($phone === '') {
        ...
        echo json_encode(['success' => false, 'error' => 'phone is required'], JSON_UNESCAPED_UNICODE);
        exit;
    }

    $onlyUnread = (string)($_GET['unread'] ?? '') === '1';
    $sql = 'SELECT id, title, body, read_flag, created_at FROM notifications WHERE user_phone = :phone';
    if ($onlyUnread) {
        $sql .= ' AND (read_flag = 0 OR read_flag IS NULL)';
    }
