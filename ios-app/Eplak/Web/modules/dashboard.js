  ...
  function renderDashboard() {
    ...
    if (activityWrap) {
      ...
      activityWrap.innerHTML = recent.length === 0
        ? `<div style="text-align:center; padding:24px 10px; color:var(--text-muted); font-size:13px;">${isEn ? 'No activities recorded yet' : 'هنوز فعالیتی ثبت نشده است'}</div>`
        : recent.map(r => {
          const statusMeta = (typeof getStatusMeta === 'function') ? getStatusMeta(r.status) : { label: r.status, className: 'status-pending' };
          return `
